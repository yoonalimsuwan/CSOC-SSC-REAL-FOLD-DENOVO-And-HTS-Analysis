#!/usr/bin/env python3
# =============================================================================
# CSOC‑SSC v30.1.1 — MSA‑Enabled Hybrid Folding Engine
# =============================================================================
...
# ──────────────── Logging ────────────────
def setup_logger(name="CSOC‑SSC_V30.1.1", local_rank=-1):
...
class V30_1_1Config:
    ...
    use_msa: bool = False                       # enable MSA encoder
    msa_n_seq: int = 128
    msa_dim: int = 64
...
class MultimerPDBFetcher:
    ...
    def fetch(pdb_id: str) -> Tuple[List[Backbone], List[str]]:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'CSOC-SSC_V30.1.1'})
        chains_data = {}
        chain_order = []
        try:
    ...
    def __init__(self, pdb_dir, msa_dir, max_len=500, max_n_seq=128):
        ...
        for msa_path in glob.glob(os.path.join(msa_dir, "*.npy")):
            ...
            try:
                ...
                for bb in backbones:
                    if 10 <= len(bb.seq) <= max_len:
                        msa = np.load(msa_path)  # (N_seq, L, 22)
...
def build_sidechain_atoms(ca, seq, chi_angles):
    ...
    for i, aa in enumerate(seq):
        ...
        for (atom_name, atom_type, parent_idx, bond_len, bond_ang_deg, ref_tuple, dihedral_ang0) in topo:
            ...
            chi_val = chi_angles[i, chi_idx]; total_angle = dihedral_ang0 + chi_val
...
# ──────────────── Energy functions (identical to v30.1) ────────────────
...
class CSOCSSC_V30_1_1(nn.Module):
    ...
    def predict_multimer(self, sequences, initial_coords_list=None, msa=None):
        self.eval()
    ...
    def refine_multimer(self, sequences, init_coords_list=None, steps=None, logger=None, msa=None):
        ...
        if init_coords_list is not None:
            ...
            with torch.no_grad():
                chains_ca, alpha_np = self.predict_multimer(sequences, msa=msa)
...
def train_model(model, dataloader, cfg, logger, use_msa=False):
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch in dataloader:
            ...
            scaler.scale(loss).backward()
            if (batch[0].size(0) % cfg.gradient_accumulation_steps) == 0:  # simplified
        ...
        for i, aa in enumerate(seq):
            ...
            if aa != 'G':
                for k in range(3, res_coords.shape[0]):
                    at_name = res_types[k]; x,y,z = res_coords[k]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CSOC‑SSC V30.1.1 – MSA‑Enabled Hybrid Folding Engine")
    ...
    refine_parser.add_argument('--msa', type=str, default=None)       # NEW: path to .npy MSA file
    ...
    cfg = V30_1_1Config(local_rank=local_rank, is_distributed=is_distributed, device=device_str,
                        use_msa=use_msa,
    ...
    logger = setup_logger("CSOC-SSC_V30.1.1", local_rank)

    if local_rank in [-1,0]:
        logger.info("="*60)
        logger.info("CSOC-SSC V30.1.1 – MSA‑Enabled Hybrid Folding Engine")
    ...
    if args.command == 'train':
        ...
        msa_tensor = None
        if args.msa:
            msa_np = np.load(args.msa)  # (N_seq, L, 22)
